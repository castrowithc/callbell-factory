...
The backlog is the repo's operational work trail. It lives in the `_backlog/` zone (see `callbell-zones`).
...
- **The `_backlog/` root is the default.** Epics, stories, and tasks live flat here (`epic-<slug>.md`,
  `story-<slug>.md`, `task-<slug>.md`) when they belong to no explicit project. Most repos need nothing more.
- **A project is an optional folder** `_backlog/<project>/` that gathers a body of related work (one or
  more epics, stories, or tasks) under one name, with an `index.md` header (below). The same flat entries
  live inside it: `_backlog/<project>/epic-<slug>.md`, and so on.
...
- **Membership is the folder, not the frontmatter.** An entry belongs to the project whose folder holds
  ...
  that grows into its own stream moves into a new `_backlog/<project>/` (a folder move, so after approval).

## Project header: `index.md`
Every project folder carries `_backlog/<project>/index.md`: the project's header and roster. It states the
...
## Overview: `BACKLOG.md`
`_backlog/BACKLOG.md` is the single overview of the whole work trail, the counterpart to the memory index
...
- A finished entry moves into the **local** `done/`: `_backlog/done/` for a root entry,
  `_backlog/<project>/done/` for a project entry. Lasting insights are filed as durable content first (the
...
## Templates
Templates for new entries live in `__META__/templates/` (`epic.md`, `story.md`, `task.md`, and the project
