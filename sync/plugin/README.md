...
(`callbell-plugin.map.*` + `scripts/callbell-plugin-sync.js`, story-factory-channel). Skills, the hook
script, rules, and the ruleset are gathered from the `sync/` scopes; only these host-specific files live
here.
...
- `hooks/callbell-hooks.json` — the shared SessionStart hook map, referenced by both manifests. It passes
  **no** `--rules`: the hook's plugin-root fallback injects rules for both hosts on its own, and only the
  Codex project hook keeps `--rules` (see story-ambient-hook).
