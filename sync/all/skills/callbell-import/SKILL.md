...
description: >
  Take raw material the user dropped in __callbell__/zone-import/ and turn it into filed knowledge. Use this whenever
  ...
  __callbell__/zone-import/ directly. Covers binaries (images, PDF, Office, exports) and text (Markdown, txt, Obsidian
...
The `__callbell__/zone-import/` zone is the inbound workbench (see `callbell-zones`): the user drops raw material there,
the agent turns it into something the repo can hold. `__callbell__/zone-import/` itself is volatile and unversioned; only
...
a fixed phrase. When such a cue appears, or the user names `__callbell__/zone-import/`, look there.

## Steps

1. **Inventory.** List what is in `__callbell__/zone-import/` (ignore the `processed/` archive). Report what you found and
...
7. **Archive the original.** Move the consumed original into `__callbell__/zone-import/processed/<yyyy-mm>/` (the month it
...
`__callbell__/zone-import/` must not grow without bound. The month buckets `__callbell__/zone-import/processed/<yyyy-mm>/` make cleanup
...
- **The filed result is redacted, the archived original is not.** Real data survives only in `__callbell__/zone-import/`
...
- **Deliverables go the other way.** Finished outputs the user asks to take out of the repo belong in
  `__callbell__/zone-export/`, not here (see `callbell-zones`).
