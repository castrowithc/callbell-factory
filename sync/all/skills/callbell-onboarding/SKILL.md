...
- **code:** point to `__META__/docs/` as the place for project documentation; the root stays the code project.
- **ops:** fill the area registry in the root `framework.md` with the named areas. Create no empty area
...
- **Structure:** the path says WHERE, the frontmatter says WHAT, `status` drives maturity. Docs in
  `__META__/docs/` (code) or flat area folders `<area>-<topic>` (ops). Zones: `_import/` inputs, `_export/`
  requested deliverables, `_backlog/` the work trail.
