...
description: >
  Take raw material the user dropped in _import/ and turn it into filed knowledge. Use this whenever
  ...
  _import/ directly. Covers binaries (images, PDF, Office, exports) and text (Markdown, txt, Obsidian
...
The `_import/` zone is the inbound workbench (see `callbell-zones`): the user drops raw material there,
the agent turns it into something the repo can hold. `_import/` itself is volatile and unversioned; only
...
a fixed phrase. When such a cue appears, or the user names `_import/`, look there.

## Steps

1. **Inventory.** List what is in `_import/` (ignore the `processed/` archive). Report what you found and
...
7. **Archive the original.** Move the consumed original into `_import/processed/<yyyy-mm>/` (the month it
...
`_import/` must not grow without bound. The month buckets `_import/processed/<yyyy-mm>/` make cleanup
...
- **The filed result is redacted, the archived original is not.** Real data survives only in `_import/`
...
- **Deliverables go the other way.** Finished outputs the user asks to take out of the repo belong in
  `_export/`, not here (see `callbell-zones`).
