...
The user may deliberately drop material with personal data into `_import/` for the agent to process (see
`callbell-zones`, `callbell-import`). `_import/` is volatile and unversioned, so the raw original stays out
