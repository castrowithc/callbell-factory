...
`Procedures are skills, not commands`: onboarding (shared, domain-adaptive) and worktree are **pure skills**. A user-invocable skill appears in Claude as `/name` **and** in Codex as `@name`, so the command UX stays intact across harnesses. The templates no longer ship a `.claude/commands/` folder. The onboarding skill carries `disable-model-invocation: true` (one-time setup, never automatic). `callbell-plan` carries it too, for a different reason: wanting a thing built and thinking a thing through look identical from the outside, so an auto-trigger would cut half-formed ideas into packages, and a loaded skill cannot be unloaded from a running session. The flag also drops the skill's description from context, so the agent would not know the skill exists: that is why the rule `callbell-backlog` names it, which lets the agent suggest it without loading it. Worktree stays without the flag so the agent may proactively suggest it during parallel work.
